...
(
    perent_id   VARCHAR(50) DEFAULT '' NOT NULL COMMENT '시작목록 ID',
    vw_cd       VARCHAR(40) NOT NULL            COMMENT '서비스뷰 ID',
    vw_nm       VARCHAR(300) NOT NULL           COMMENT '서비스뷰명',
    list_id     VARCHAR(40) DEFAULT '' NOT NULL COMMENT '목록 ID',
    list_nm     VARCHAR(300)                    COMMENT '목록명',
    org_id      VARCHAR(40)                     COMMENT '제공기관 코드',
    tbl_id      VARCHAR(40) DEFAULT '' NOT NULL COMMENT '통계표 ID',
    ...
    CONSTRAINT mt_kosis_stat_pkey PRIMARY KEY (perent_id, vw_cd, list_id, tbl_id)
);

ALTER TABLE ecodi_meta.mt_kosis_stat COMMENT = 'KOSIS 통계목록 계층구조';
